...
```{r c_des_tcbil, fig.align='center'}
# Calcular el promedio de puntajes por tipo de colegio
promedios_bilingue <- data %>%
  filter(!is.na(COLE_BILINGUE) & COLE_BILINGUE %in% c('S', 'N')) %>%
...
# Reorganizar los datos para el gráfico de barras apiladas
stacked_data_bilingue <- promedios_bilingue %>%
  pivot_longer(cols = starts_with("Promedio"), names_to = "Area", values_to = "Promedio")

# Crear el gráfico de barras apiladas
ggplot(stacked_data_bilingue, aes(x = COLE_BILINGUE, y = Promedio, fill = Area)) +
  geom_bar(stat = "identity", position = "stack") +
  geom_text(aes(label = round(Promedio, 2)), 
            position = position_stack(vjust = 0.5), 
            color = "white", size = 4) +
  labs(title = "Desempeño promedio por área y tipo de colegio bilingüe",
       x = "Tipo de colegio",
       y = "Puntaje promedio") +
  theme_minimal() +
  theme(legend.position = "right")
...
# Calcular correlación entre puntajes y tipo de colegio
correlacion_bil <- cor(data$PUNT_GLOBAL, as.numeric(data$COLE_BILINGUE == "S"))

# Calcular correlación entre puntajes y tipo de colegio
correlacion_no_bil <- cor(data$PUNT_GLOBAL, as.numeric(data$COLE_BILINGUE == "N"))

# Mostrar la correlación
cat("La correlación entre tipo de colegio bilingüe y el puntaje es:", correlacion_bil)

cat("La correlación entre tipo de colegio no bilingüe y el puntaje es:", correlacion_no_bil)
```
