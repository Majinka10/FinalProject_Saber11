...
La gráfica revela diferencias en desempeño por tipo de colegio (oficial/no oficial). 
Indica una brecha en puntajes, sugiriendo disparidades educativas y socioeconómicas.
...
Se observa que la diferencia en el desempeño promedio por área entre colegios oficiales
y no oficiales sugiere una desigualdad significativa en la calidad educativa entre
estos dos tipos de colegios. Los puntajes más altos en colegios no oficiales,
especialmente en inglés, matemáticas y lectura crítica, indican una posible brecha
en la calidad educativa entre los dos tipos de instituciones.
...
cat("La correlación entre tipo de colegio privado y el puntaje es:", correlacion_privado)

cat("La correlación entre tipo de colegio público y el puntaje es:", correlacion_publico)
```
...
print(paste("La correlación entre el nivel educativo del padre y el puntaje es:", round(correlation_padre, 3)))
print(paste("La correlación entre el nivel educativo de la madre y el puntaje es:", round(correlation_madre, 3)))
```
...
correlacion_con_pc <- cor(data$PUNT_GLOBAL, as.numeric(data$FAMI_TIENECOMPUTADOR == "Si"))

# Calcular correlación entre puntajes y tener computadora
correlacion_sin_pc <- cor(data$PUNT_GLOBAL, as.numeric(data$FAMI_TIENECOMPUTADOR == "No"))
...
# Calcular correlación entre puntajes y tener computadora
correlacion_con_internet <- cor(data$PUNT_GLOBAL, as.numeric(data$FAMI_TIENEINTERNET == "Si"))

# Calcular correlación entre puntajes y tener computadora
correlacion_sin_internet <- cor(data$PUNT_GLOBAL, as.numeric(data$FAMI_TIENEINTERNET == "No"))
```

```{r c_internet_des, fig.align='center'}
# Gráfico 1: Puntajes según el acceso a un computador
plot_pc <- ggplot(combined_data_pc, aes(x = TieneComputadora, y = PUNT_GLOBAL, fill = TieneComputadora)) +
  geom_boxplot() +
  labs(title = NULL,
       x = "Tiene computador",
       y = "Puntaje global") +
...
plot_internet <- ggplot(combined_data_internet, aes(x = TieneInternet, y = PUNT_GLOBAL, fill = TieneInternet)) +
  geom_boxplot() +
  labs(title = NULL,
       x = "Tiene Internet",
       y = "Puntaje global") +
...
# Mostrar la correlación
cat("La correlación entre tener computador y el puntaje es:", correlacion_con_pc)

cat("La correlación entre no tener computador y el puntaje es:", correlacion_sin_pc)

# Mostrar la correlación
cat("La correlación entre tener Internet y el puntaje es:", correlacion_con_internet)

cat("La correlación entre no tener Internet y el puntaje es:", correlacion_sin_internet)
```

### Uso de tiempo {.tabset}

#### Tiempo dedicado a Internet
¿El tiempo que pasan los jovenes en internet afecta a su puntaje en las pruebas ICFES?
...
dentro del condicional cambiando los valores numéricos por factores.<br>
Posterior a esto, se hace la gráfica de "dedicacion_internet" vs "punt_global" con la 
función geom_boxplot de ggplot. <br>
...
##### Qué se observa
No hay una gran diferencia en los puntajes promedio de las personas que no leen por entretenimiento y las que leen 30 minutos o menos. la media de ambos varía de 245 puntos a 248.

De igual forma, la diferencia entre leer 30 a 60 minutos y leer entre 1 y 2 horas no es muy significativa. Incluso se puede observar un muy ligero descenso, siendo la media de 263 y 261, respectivamente.

Mientras que la media de puntajes de quienes leen más de 2 horas diarias es de 270 puntos.
...
Esta pregunta comparte procedimiento con el análisis de las preguntas anteriores: <br>
se hace la conversión a factor de los valores guardados en la columna 'horas_trabajo.<br>
Se crea la gráfica teniendo de base las horas de trabajo y el puntaje global (como X y Y) de matrix_df.<br>
